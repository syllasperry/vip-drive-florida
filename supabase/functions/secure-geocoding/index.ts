...
function checkRateLimit(ip: string, maxRequests: number = 30, windowMs: number = 60000): boolean {
  ...
  if (record.count >= maxRequests) {
    ...
    if (record.violations > 3) {
      record.resetTime = now + (windowMs * 2); // Double penalty
    }
    return false;
  }
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 Geocoding request from IP: ${clientIP}, Query: "${sanitizedQuery.substring(0, 50)}..."`);
    ...
    if (!mapsResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ 
          error: 'External geocoding service temporarily unavailable',
    ...
    if (!data || typeof data !== 'object') {
      ...
      return new Response(
        JSON.stringify({ error: 'Invalid response from geocoding service' }),
        { status: 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // Log successful request
    console.log(`✅ Geocoding success for IP: ${clientIP}, Results: ${data.predictions?.length || 0}`);
