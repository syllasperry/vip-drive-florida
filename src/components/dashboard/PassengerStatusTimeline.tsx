...
export const PassengerStatusTimeline = ({ bookings, onReopenModal }: PassengerStatusTimelineProps) => {
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      ...
      case 'booking_requested': return 'Waiting for Driver';
          ...
          <CardContent className="space-y-4">
            ...
            {booking.vehicle_type && (
              <div className="flex items-center gap-2 p-2 bg-blue-50 rounded-lg">
                ...
                <span className="text-sm font-medium">{booking.vehicle_type}</span>
              </div>
            )}
            ...
            {booking.drivers && (
              <div className="flex items-center gap-2 p-2 bg-green-50 rounded-lg">
                ...
                {booking.drivers.phone && (
                  <span className="text-sm text-gray-500">• {booking.drivers.phone}</span>
                )}
              </div>
            )}
